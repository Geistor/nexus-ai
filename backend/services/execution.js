const { venueAllocationMultiplier } = require('./portfolioLevelRiskManager');
const { evaluateExecutionGuards } = require('./liveSafetyGuards');
const { allocationMultiplierFromPlan } = require('./capitalAllocatorAI');
...
async function executeDecision(ctx, symbol, decision, features) {
  ...
  const venueMult = venueAllocationMultiplier(ctx.state.portfolioManager?.venueCapitalPlan || [], venue.exchangeName);
  const profitExecutionProfile = ctx.state.profitState?.executionProfiles?.[symbol] || null;
  const allocMult = allocationMultiplier(ctx, symbol, decision, features) * allocationMultiplierFromPlan(ctx.state.capitalPlan, symbol) * venueMult;
  ...
  const guardResult = evaluateExecutionGuards(ctx, symbol, venue, decision, features, size);
  ctx.state.liveSafety = { stage: guardResult.stage, config: guardResult.config, lastGuardResult: guardResult };
  if (!guardResult.passed) {
    ctx.helpers.pushLog('WARN', 'Execution guard blokkolta a kötést', { symbol, venue: venue.exchangeName, failures: guardResult.failures, diagnostics: guardResult.diagnostics });
    return;
  }
