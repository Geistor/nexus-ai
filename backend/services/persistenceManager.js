...
function persistTradeEvent(ctx, payload) {
  ...
  remote.insertTradeEvent(payload).catch(() => {});
}
...
function updateLearningState(ctx, patch = {}) {
  ...
  remote.upsertSystemState('learning_state', merged).catch(() => {});
  return merged;
}
...
function registerModelVersion(ctx, meta) {
  ...
  remote.upsertModelRegistryEntry(meta).catch(() => {});
  return current;
}

function setActiveModelVersion(ctx, version, meta = {}) {
  const p = ensureStructure(ctx);
  const active = { version, ...meta, activatedAt: new Date().toISOString() };
  safeWriteJson(p.activeModelFile, active);
  remote.upsertSystemState('active_model', active).catch(() => {});
...
function createPersistenceSnapshot(ctx, label = 'manual') {
  ...
  const filesToUpload = [
    ...
    ['manifest.json', p.manifestFile],
    ['models/active.json', p.activeModelFile],
    ['models/registry.json', path.join(p.modelsDir, 'registry.json')]
  ];
  Promise.all(filesToUpload.map(([r, l]) => remote.uploadFile(l, `snapshots/${path.basename(snapshotDir)}/${r}`))).catch(() => {});
  return { snapshotDir };
}
